...
if [[ $reject1 ]]; then
 reject1=`echo $reject1 | sed "s|\"| |g" `
 REJECTSYMB1=`echo $reject1 | awk '{print $1}' `
...
if [[ $reject2 ]]; then
 reject2=`echo $reject2 | sed "s|\"| |g" `
 REJECTSYMB2=`echo $reject2 | awk '{print $1}' `
...
##
 ...
 STAT=`echo $?`
 if [[ $STAT -ne 0 ]]; then
    echo " Python script ended with an error!"
    REJECT="ON"
 fi
 if [[ $verbosity -gt 4 ]]; then
  echo " Done."
